...
Whatever predicate you put as http_handler's second (ie :Closure) argument in turn has a hidden final argument &mdash; conventionally called *Request* &mdash; which gets passed to your handler as its final argument. In the static files examples in the next section which use a library predicate, [http_reply_from_files(+Dir, +Options, +Request)](http://www.swi-prolog.org/pldoc/doc_for?object=http_reply_from_files/3), we can ignore *Request*, but it plays a leading role in subsequent examples when we write our own handlers.
